...
public sealed class PixelComparatorTests : IDisposable
{
    ...
    {
        Write(Name(ParityLeg.React, "0"), 4, 4);
        Write(Name(ParityLeg.BlazorServer, "0"), 4, 4, (2, 1, SKColors.Black));
        ...
        finding.Property.ShouldBe("0");
        finding.ReferenceValue.ShouldBe(Name(ParityLeg.React, "0"));
        finding.CandidateValue.ShouldBe(Name(ParityLeg.BlazorServer, "0"));
    ...
    public void MarksTheDifferingPixelRedOverAFadedReference()
    {
        ...
        using var diff = Read(ScreenshotSet.DiffName(Name(ParityLeg.BlazorServer, "0")));
    ...
    public void WritesNoDiffWhenTheFractionIsWithinTheThreshold()
    {
        ...
        File.Exists(Path.Combine(directory, ScreenshotSet.DiffName(Name(ParityLeg.BlazorServer, "0"))))
            .ShouldBeFalse();
    }
    ...
    {
        Write(Name(ParityLeg.React, "0"), 4, 4, (1, 1, SKColors.Black));
        Write(Name(ParityLeg.BlazorServer, "0"), 4, 4, (1, 1, SKColors.Black));
    ...
    {
        Write(Name(ParityLeg.React, "0"), 4, 4);
        Write(Name(ParityLeg.BlazorServer, "0"), 4, 4, (0, 0, new SKColor(247, 255, 255, 255)));
    ...
    {
        Write(Name(ParityLeg.React, "0"), 4, 4);
        Write(Name(ParityLeg.BlazorServer, "0"), 4, 4, (0, 0, new SKColor(246, 255, 255, 255)));
    ...
    {
        Write(Name(ParityLeg.React, "0"), 4, 4);
        Write(Name(ParityLeg.BlazorServer, "0"), 4, 4, (0, 0, new SKColor(249, 249, 249, 255)));
    ...
    {
        Write(Name(ParityLeg.React, "0"), 4, 4);
        Write(Name(ParityLeg.BlazorServer, "0"), 4, 4, (0, 0, new SKColor(255, 255, 255, 200)));
    ...
    {
        Write(Name(ParityLeg.React, "0"), 4, 4);
        Write(Name(ParityLeg.BlazorServer, "0"), 4, 5);
    ...
    {
        Write(Name(ParityLeg.React, "0"), 4, 4);
        Write(Name(ParityLeg.React, "1"), 4, 4);
        Write(Name(ParityLeg.BlazorServer, "0"), 4, 4);

        var context = Context(
            threshold: 0.5,
            reference: [Name(ParityLeg.React, "0"), Name(ParityLeg.React, "1")],
            candidate: [Name(ParityLeg.BlazorServer, "0")]);

        // Shot 1 is portal(1): React portalled a popup out and Blazor did not. Dropping
        ...
        finding.Property.ShouldBe("1");
        finding.ReferenceValue.ShouldBe(Name(ParityLeg.React, "1"));
    ...
    public void ReportsAScreenshotThatIsNotOnDisk()
    {
        ...
        finding.Message.ShouldContain(Name(ParityLeg.BlazorServer, "0"));
    }

    [Fact]
    public void ComparesEverySettledShotAndEveryAnimationFrame()
    {
        string[] shots = ["0", "1", "frame0.0", "frame100.1"];
        ...
        findings.Select(f => f.Property).ShouldBe(["0", "1", "frame0.0", "frame100.1"], ignoreOrder: true);
    }
    ...
    {
        var name = ScreenshotSet.Name(Fixture, ParityLeg.BlazorWasm, "open", "frame25.1");

        name.ShouldBe("select__grouped.BlazorWasm.open.frame25.1.png");
        ...
        ScreenshotSet.Shot(name, Fixture, ParityLeg.BlazorWasm, "open").ShouldBe("frame25.1");
    }
    ...
    {
        ScreenshotSet.DiffName("select__grouped.BlazorServer.initial.0.png")
            .ShouldBe("select__grouped.BlazorServer.initial.0.diff.png");
    }

    private IEnumerable<Finding> Compare(double threshold)
        => new PixelComparator(directory).Compare(Context(
            threshold,
            [Name(ParityLeg.React, "0")],
            [Name(ParityLeg.BlazorServer, "0")]));
