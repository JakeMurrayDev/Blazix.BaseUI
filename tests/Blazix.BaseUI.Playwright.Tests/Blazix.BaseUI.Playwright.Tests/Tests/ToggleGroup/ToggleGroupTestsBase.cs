...
public abstract class ToggleGroupTestsBase : TestBase
{
    ...
    protected async Task WaitForToggleGroupJsAsync()
    {
        await Page.WaitForFunctionAsync(@"() => {
            ...
            if (!el) return false;
            const stateKey = Symbol.for('Blazix.BaseUI.ToggleGroup.State');
            ...
            const itemKey = Symbol.for('Blazix.BaseUI.ToggleGroupItem.State');
    ...
    protected async Task WaitForToolbarJsAsync()
    {
        await Page.WaitForFunctionAsync(@"() => {
            ...
            if (!el) return false;
            const stateKey = Symbol.for('Blazix.BaseUI.Toolbar.State');
            const map = window[stateKey];
            if (!map || !map.has(el)) return false;
