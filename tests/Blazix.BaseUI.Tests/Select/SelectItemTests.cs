...
public class SelectItemTests : BunitContext, ISelectItemContract
{
    ...
    public Task ShouldNotEmitDataLabel()
    {
        ...
        var items = cut.FindAll("[role='option']");
        // React's SelectItem does not emit `data-label`; we use `data-blazix-base-ui-label` instead.
    ...
    public Task ShouldEmitDataBlazixBaseUiLabelWhenLabelSet()
    {
        ...
        var items = cut.FindAll("[role='option']");
        items[0].GetAttribute("data-blazix-base-ui-label").ShouldBe("Apple");
        // Second item has no Label parameter → attribute absent
        items[1].HasAttribute("data-blazix-base-ui-label").ShouldBeFalse();
