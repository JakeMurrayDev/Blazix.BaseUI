/**
 * Blazix.BaseUI Floating Infrastructure
 *
...
const STATE_KEY = Symbol.for('Blazix.BaseUI.Floating.State');
...
if (!document.querySelector('[data-baseui-positioner-css]')) {
    ...
    style.textContent = [
        '[data-blazix-base-ui-positioner][role="presentation"][data-side]:not([data-positioned]) { visibility: hidden !important; position: fixed !important; }',
...
const FLOATING_UI_KEY = Symbol.for('Blazix.BaseUI.FloatingUI');
...
async function ensureFloatingUI() {
    ...
    state.floatingUIPromise = (async () => {
        const basePath = './_content/Blazix.BaseUI/vendor/';
...
export function getTabbableElements(container) {
    ...
    return elements.filter(el => {
        if (el.hasAttribute('data-blazix-base-ui-focus-guard')) return false;
...
export function initializeFocusGuards(options) {
    ...
    const guards = floatingElement.parentElement?.querySelectorAll('[data-blazix-base-ui-focus-guard]');
    ...
    function walkAndMark(parent) {
        for (const child of parent.children) {
            ...
            if (currentCount === 0) {
                ...
                child.setAttribute('data-blazix-base-ui-inert', '');
            }
    ...
    return function cleanup() {
        for (const child of markedElements) {
            ...
            if (newMarkerCount <= 0) {
                ...
                child.removeAttribute('data-blazix-base-ui-inert');
            } else {
...
export const NAVIGABLE_ITEM_SELECTOR =
    ...
    '[role="menuitemradio"], [data-blazix-base-ui-list-item]';
...
export function applyActiveState(listElement, itemElement, virtual) {
    ...
    const prev = listElement.querySelector('[data-blazix-base-ui-active]');
    if (prev) {
        prev.removeAttribute('data-blazix-base-ui-active');
        if (!virtual) prev.setAttribute('tabindex', '-1');
    }

    if (itemElement) {
        itemElement.setAttribute('data-blazix-base-ui-active', '');
        if (virtual) {
...
export function isThirdPartyElement(target, floatingElement) {
    ...
    const inertMarkers = document.querySelectorAll('[data-base-ui-inert], [data-blazix-base-ui-inert]');
    ...
    let current = target;
    while (current && current !== document.body) {
        if (current.hasAttribute('data-base-ui-inert') || current.hasAttribute('data-blazix-base-ui-inert')) return false;
        current = current.parentElement;
    }
...
const DELAY_GROUP_KEY = Symbol.for('Blazix.BaseUI.DelayGroup.State');
