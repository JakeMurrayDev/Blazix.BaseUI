...
- **`DialogOpenChangeReason.Swipe`/`CloseWatcher`** are never emitted by the Dialog itself, but they are not dead code: the Drawer (which shares `DialogOpenChangeReason`) emits them — `DrawerSwipeArea`/`DrawerViewport` produce `Swipe`, and the drawer JS fires `CloseWatcher`. Left intact as shared reasons.
