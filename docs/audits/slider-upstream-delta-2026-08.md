...
| Upstream | (commit, Slider) verdict | User-observable symptom | Evidence | Verified against |
|---|---|---|---|---|
| `db86f44d8` #5390 | **(d:already-present)** via Field | A Slider inside a Field whose custom error was set and cleared kept reporting the stale required-validity. | Landed PR #206; a Slider inside a Field inherits the fixed behavior with no Slider-side code. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
| `d060189d4` #5290 | **(d:already-present)** via Field | A Slider inside a Field with a null control value reported the wrong `data-dirty` state. | Landed PR #206; inherited with no Slider-side code. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
| `293a0f1ed` #5287 | **(d:already-present)** via Form | Submitting a form focused the first invalid field in registration order rather than document order, so focus could jump past a Slider that appears earlier on the page. | Landed PR #206; inherited with no Slider-side code. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
| `8846f9937` #5383 | **(a) skip** | No user-observable Slider symptom: the fix covers form serialization of portalled `CheckboxGroup`/`RadioGroup` values. | Slider renders no group and contributes a single named input per thumb, so it has no portalled group value to serialize. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
| `32cabb778` #5281 | **(a) skip — React-specific** | No runtime content: `FieldError`/`FieldItem`/`FieldsetLegend` cleanup alongside tests. | Field-owned; nothing in Slider's rendered output, ARIA or interaction changes. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
| `595c0fa08` #5340 | **defer-with-spec → [#158](https://github.com/JakeMurrayDev/Blazix.BaseUI/issues/158) batch 2** | Swapping the slider's label lets the outgoing instance clear the incoming one's registered id, so the slider loses its accessible name — `aria-labelledby` points at nothing. Under Blazor this is the common case rather than a race, because a removed component is disposed after its replacement has initialized. | `Slider/SliderLabel.razor:119` still clears the registered id unconditionally on dispose. Deferred rather than skipped (Resolving uncertainty, tier 2; Q6.3 — "a recorded debt, not a skip"); the debt is owned by #158, which tracks this site. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
| `1a2ca3c9f` #5401 | **(d:moot)** | Interrupting a closing popup released its locked size early, so the replacement animation started from a collapsed box. | Slider renders no popup, opens no floating element, has no popup handle and runs no list navigation. This commit reaches Slider's import graph only through shared barrel re-exports (`internals/*`, `floating-ui-react/*`), not through any executed Slider path, so the symptom has no local site. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
...
| `54cfcc188` #5386 | **(a) skip — React-specific** | No user-observable symptom: TypeScript declaration emission for published internals. | No runtime content; nothing in DOM output, ARIA, focus order, keyboard/pointer behavior, timing constants or the public API surface changes. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
| `ee38be3e2` #5250 | **(a) skip — React-specific** | No user-observable symptom: store-selector code size. | Shipped bytes only; the rendered Slider output is identical. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
| `006a72a99` #5341 | **(a) skip — React-specific** | No user-observable symptom observed: the commit relocates which React component owns a cleanup effect. | Recorded **revisit-on-symptom** per #158; no Slider divergence in value-commit or drag teardown ordering has been observed to date. | `4b2a7923` + `1a2ca3c9f` · 2026-08-21 |
