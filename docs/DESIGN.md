...
- The liquid prism pair lives in `App.razor`: `#liquid-prism-intense` (single displacement pass, scale 200, used as a `backdrop-filter` so the content behind the glass refracts — Chromium; others fall back to the translucent fill) and `#liquid-prism-intense-2` (two-stage displacement chain, scale 30 then 20, used as an element `filter` on hover). `liquid-wobble` adds the gel press animation. Reserve the trio for playful one-off controls like the demo code expander; elements using them apply border radius inline (e.g. `rounded-full`), not via shared panel classes. Everything falls back to the plain control under `prefers-reduced-motion`. Both variants can be compared side by side in `wwwroot/liquid-glass-preview.html`.
