...
- Use light translucent surfaces, white borders, layered shadows, and `backdrop-blur`. Tinting is theme-specific: the light theme keeps its restrained cyan/sky tints; the dark theme is monochrome (see the next rule).
- Dark theme is black-and-white: pure-black background with a neutral grayscale wallpaper, neutral white-alpha glass, and no color accents — chrome, content surfaces, code blocks, and active states all resolve to black/white or `neutral`-palette grays (not `slate`, which carries a blue cast). Apply this in dark mode only via `dark:` variants; the light theme keeps its color. Live component demos retain their own styling.
...
- Opening and closing motion should use the component state attributes emitted by Blazix.BaseUI: `data-starting-style`, `data-ending-style`, `data-open`, and `data-closed`. Drawers slide and fade from the edge, popups scale/fade from their placement origin, and collapsible panels animate height with a small opacity/position shift. Respect reduced motion by disabling transitions and keyframes.
