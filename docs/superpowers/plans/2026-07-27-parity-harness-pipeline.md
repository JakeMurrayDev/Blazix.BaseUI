...
- pnpm 11 refuses to run scripts for dependencies with unapproved install scripts and no longer reads the `pnpm` field from `package.json`. `react-fixtures/pnpm-workspace.yaml` carries `onlyBuiltDependencies` (`esbuild: true`, `@parcel/watcher: false`); it is required for `pnpm parity:css` and `parity:build` to run at all.
