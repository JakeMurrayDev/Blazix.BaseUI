...
- Upstream-delta audits must classify every upstream commit per `docs/audits/METHODOLOGY.md`: four verdict classes — skip (React-specific), transfer to C#/Razor, transfer to JS module, no-op (sub-labeled `already-present` or `moot`). Verdicts attach to (commit, component) pairs; skip/no-op verdicts require positive verification with a user-observable symptom restatement, never a mechanism name alone; uncertain cases default to port. Record the standard disposition row (SHA + PR, verdict, symptom, evidence, local HEAD + pin + date) in the component's audit doc.
